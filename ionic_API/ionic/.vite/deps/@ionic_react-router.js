...
import "./react-dom-Do5OZO7D.js";
import { $t as ViewLifeCycleManager, Jt as LocationHistory, Qt as StackContext, Xt as NavManager, Zt as RouteManagerContext, bt as IonRoute, en as ViewStacks, nn as getConfig, tn as generateId } from "./dist-BsaSk9_N.js";
import "./p-BJoMtgfR-D9rUZpa5.js";
import "./p-9VcRUwdB-CR1KLRpo.js";
import "./p-DgbT0exM-DhwO7jTN.js";
import "./p-CBzELu-H-CPUqh-_T.js";
import "./p-CU1SSH8_-C3prkbcs.js";
import "./p-Cl0B-RWe-DyB7mqIA.js";
import "./p-vEbVo2hO-Cj63OdEB.js";
import "./p-D13Eaw-8-CmPZk2Lo.js";
import "./p-C59ryAuS-DBRIFaDI.js";
import "./p-D6Ynv7Xh-Dv5bGX4n.js";
import { a as Router, b as createBrowserHistory, d as matchPath$1, g as withRouter, i as Route, x as createHashHistory } from "./react-router-Djp52scH.js";
import "./react-router-dom.js";
...
//#endregion
//#region node_modules/@ionic/react-router/dist/index.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var IonRouteInner = class extends import_react.PureComponent {
	render() {
		return import_react.createElement(Route, Object.assign({
...
*/
var matchPath = ({ pathname, componentProps }) => {
	const { exact, component } = componentProps;
	const match = matchPath$1(pathname, {
		exact,
		path: componentProps.path || componentProps.from,
		component
	});
...
var ReactRouterViewStack = class extends ViewStacks {
	...
	getChildrenToRender(outletId, ionRouterOutlet, routeInfo) {
		...
		import_react.Children.forEach(ionRouterOutlet.props.children, (child) => {
			const viewItem = viewItems.find((v) => {
				return matchComponent$1(child, v.routeData.childProps.path || v.routeData.childProps.from);
			});
			if (viewItem) viewItem.reactElement = child;
		});
		return viewItems.map((viewItem) => {
			let clonedChild;
			if (viewItem.ionRoute && !viewItem.disableIonPageManagement) clonedChild = import_react.createElement(ViewLifeCycleManager, {
				key: `view-${viewItem.id}`,
				mount: viewItem.mount,
				removeView: () => this.remove(viewItem)
			}, import_react.cloneElement(viewItem.reactElement, { computedMatch: viewItem.routeData.match }));
			else {
				const match = matchComponent$1(viewItem.reactElement, routeInfo.pathname);
				clonedChild = import_react.createElement(ViewLifeCycleManager, {
					key: `view-${viewItem.id}`,
					mount: viewItem.mount,
					removeView: () => this.remove(viewItem)
				}, import_react.cloneElement(viewItem.reactElement, { computedMatch: viewItem.routeData.match }));
	...
	findViewItemByRouteInfo(routeInfo, outletId, updateMatch) {
		...
		if ((updateMatch === void 0 || updateMatch === true) && viewItem && match) viewItem.routeData.match = match;
		return viewItem;
	}
...
function clonePageElement(leavingViewHtml) {
	let html;
	if (typeof leavingViewHtml === "string") html = leavingViewHtml;
	else html = leavingViewHtml.outerHTML;
	if (document) {
		const newEl = document.createElement("div");
		newEl.innerHTML = html;
		newEl.style.zIndex = "";
		const ionBackButton = newEl.getElementsByTagName("ion-back-button");
		if (ionBackButton[0]) ionBackButton[0].remove();
		return newEl.firstChild;
	}
}
var isViewVisible = (el) => !el.classList.contains("ion-page-invisible") && !el.classList.contains("ion-page-hidden");
var StackManager = class extends import_react.PureComponent {
	constructor(props) {
		super(props);
 ...
 /**
		...
		*/
		this.pendingPageTransition = true;
			...
			if (!leavingViewItem && routeInfo.prevRouteLastPathname) leavingViewItem = this.context.findViewItemByPathname(routeInfo.prevRouteLastPathname, this.id);
			if (leavingViewItem) {
				if (routeInfo.routeAction === "replace") leavingViewItem.mount = false;
				else if (!(routeInfo.routeAction === "push" && routeInfo.routeDirection === "forward")) {
					if (routeInfo.routeDirection !== "none" && enteringViewItem !== leavingViewItem) leavingViewItem.mount = false;
				} else if ((_a = routeInfo.routeOptions) === null || _a === void 0 ? void 0 : _a.unmount) leavingViewItem.mount = false;
			}
			const enteringRoute = matchRoute((_b = this.ionRouterOutlet) === null || _b === void 0 ? void 0 : _b.props.children, routeInfo);
			if (enteringViewItem) enteringViewItem.reactElement = enteringRoute;
			...
			if (enteringViewItem && enteringViewItem.ionPageElement) {
				...
				*/
				if (enteringViewItem === leavingViewItem) {
					...
					*/
					if (enteringViewItem.routeData.match.url !== routeInfo.pathname) return;
				}
				/**
				...
				*/
				if (!leavingViewItem && this.props.routeInfo.prevRouteLastPathname) leavingViewItem = this.context.findViewItemByPathname(this.props.routeInfo.prevRouteLastPathname, this.id);
				/**
				* If the entering view is already visible and the leaving view is not, the transition does not need to occur.
				*/
				if (isViewVisible(enteringViewItem.ionPageElement) && leavingViewItem !== void 0 && !isViewVisible(leavingViewItem.ionPageElement)) return;
				/**
	...
	async setupRouterOutlet(routerOutlet) {
		const canStart = () => {
			...
			if (!(config && config.get("swipeBackEnabled", routerOutlet.mode === "ios"))) return false;
			...
			return !!enteringViewItem && enteringViewItem.mount && enteringViewItem.routeData.match.path !== routeInfo.pathname;
		};
		const onStart = async () => {
			const { routeInfo } = this.props;
			const propsToUse = this.prevProps && this.prevProps.routeInfo.pathname === routeInfo.pushedByRoute ? this.prevProps.routeInfo : { pathname: routeInfo.pushedByRoute || "" };
			...
			* via a swipe gesture.
			*/
			if (enteringViewItem && leavingViewItem) await this.transitionPage(routeInfo, enteringViewItem, leavingViewItem, "back", true);
		...
		const onEnd = (shouldContinue) => {
			if (shouldContinue) {
				...
				*/
				if (enteringViewItem !== this.context.findViewItemByRouteInfo(routeInfo, this.id, false) && (enteringViewItem === null || enteringViewItem === void 0 ? void 0 : enteringViewItem.ionPageElement) !== void 0) {
 ...
 /**
		...
		if (enteringViewItem && enteringViewItem.ionPageElement && this.routerOutletElement) if (leavingViewItem && leavingViewItem.ionPageElement && enteringViewItem === leavingViewItem) if (matchComponent(leavingViewItem.reactElement, routeInfo.pathname, true)) {
		...
		} else await runCommit(enteringViewItem.ionPageElement, void 0);
		else {
	...
	render() {
		...
		const ionRouterOutlet = import_react.Children.only(children);
		this.ionRouterOutlet = ionRouterOutlet;
		const components = this.context.getChildrenToRender(this.id, this.ionRouterOutlet, this.props.routeInfo, () => {
			this.forceUpdate();
		});
		return import_react.createElement(StackContext.Provider, { value: this.stackContextValue }, import_react.cloneElement(ionRouterOutlet, { ref: (node) => {
			if (ionRouterOutlet.props.setRef) ionRouterOutlet.props.setRef(node);
			if (ionRouterOutlet.props.forwardedRef) ionRouterOutlet.props.forwardedRef.current = node;
...
function matchRoute(node, routeInfo) {
	...
	import_react.Children.forEach(node, (child) => {
		if (!(child.props.path || child.props.from)) matchedNode = child;
	});
	return matchedNode;
}
...
var IonRouterInner = class extends import_react.PureComponent {
	constructor(props) {
		super(props);
	...
	handleChangeTab(tab, path, routeOptions) {
		...
		} else this.handleNavigate(pathname, "push", "none", void 0, routeOptions, tab);
	}
	handleHistoryChange(location, action) {
		var _a, _b, _c;
		let leavingLocationInfo;
		if (this.incomingRouteParams) if (this.incomingRouteParams.routeAction === "replace") leavingLocationInfo = this.locationHistory.previous();
		else leavingLocationInfo = this.locationHistory.current();
		else leavingLocationInfo = this.locationHistory.current();
		if (leavingLocationInfo.pathname + leavingLocationInfo.search !== location.pathname) {
			if (!this.incomingRouteParams) {
				if (action === "REPLACE") this.incomingRouteParams = {
					routeAction: "replace",
					routeDirection: "none",
					tab: this.currentTab
				};
				if (action === "POP") {
					...
					} else this.incomingRouteParams = {
						routeAction: "pop",
						routeDirection: "none",
						tab: this.currentTab
					};
				}
				if (!this.incomingRouteParams) this.incomingRouteParams = {
					routeAction: "push",
			...
			if ((_c = this.incomingRouteParams) === null || _c === void 0 ? void 0 : _c.id) {
				...
				routeInfo = Object.assign(Object.assign({ id: generateId("routeInfo") }, this.incomingRouteParams), {
	...
	*/
	...
	handleNavigate(path, routeAction, routeDirection, routeAnimation, routeOptions, tab) {
		...
		if (routeAction === "push") this.props.history.push(path);
				...
				*/
				...
				if (routeInfo.lastPathname === routeInfo.pushedByRoute || prevInfo.pathname === routeInfo.pushedByRoute && routeInfo.tab === "" && prevInfo.tab === "") {
					...
					*/
					const history = this.props.history;
					(history.goBack || history.back)();
				} else this.handleNavigate(prevInfo.pathname + (prevInfo.search || ""), "pop", "back", incomingAnimation);
			} else this.handleNavigate(defaultHref, "pop", "back", routeAnimation);
		} else this.handleNavigate(defaultHref, "pop", "back", routeAnimation);
	}
	...
	render() {
		return import_react.createElement(RouteManagerContext.Provider, { value: this.routeMangerContextState }, import_react.createElement(NavManager, {
...
var IonReactRouter = class extends import_react.Component {
	constructor(props) {
		super(props);
	...
	*/
	handleHistoryChange(location, action) {
		...
		if (this.historyListenHandler) this.historyListenHandler(locationValue, actionValue);
	}
	...
	render() {
		...
		return import_react.createElement(Router, Object.assign({ history: this.history }, props), import_react.createElement(IonRouter, { registerHistoryListener: this.registerHistoryListener }, children));
	}
};
var IonReactMemoryRouter = class extends import_react.Component {
	constructor(props) {
		super(props);
	...
	*/
	...
	render() {
		const _a = this.props, { children } = _a, props = __rest(_a, ["children"]);
		return import_react.createElement(Router, Object.assign({}, props), import_react.createElement(IonRouter, { registerHistoryListener: this.registerHistoryListener }, children));
	}
};
var IonReactHashRouter = class extends import_react.Component {
	constructor(props) {
		super(props);
		const { history } = props, rest = __rest(props, ["history"]);
